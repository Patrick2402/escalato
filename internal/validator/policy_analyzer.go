...
func analyzeStatements(statements []Statement, serviceToCheck, actionToCheck string, policyName string) (bool, string, models.Confidence) {
	...
	for i, stmt := range statements {
		...
		exactMatch := false
		serviceHasWildCardPermissions := false
		for _, action := range actions {
			...
			if action == serviceToCheck+":*" || action == serviceToCheck+":*"+actionToCheck {
				exactMatch = true
				serviceHasWildCardPermissions = true
				if EnableDiagnostics {
					fmt.Fprintf(os.Stderr, "[DIAG-ANALYZER] Found wildcard match for %s\n", action)
				}
				break
			}
		}
		...
		if exactMatch {
			...
			if serviceHasWildCardPermissions {
				details = fmt.Sprintf("Policy '%s' contains wildcard service permissions for %s", displayPolicyName, exactMatchToFind)
			} else {
				details = fmt.Sprintf("Policy '%s' contains exact match for %s", displayPolicyName, exactMatchToFind)
			}
			...
			if EnableDiagnostics {
				fmt.Fprintf(os.Stderr, "[DIAG-ANALYZER] Wildcard violation found: %s\n", details)
			}
